...
function CheckoutPage() {
    const navigate = useNavigate()
    const { cart, totalAmount, totalAfterVoucher, voucherDiscount, shippingFee, shop, selectedVoucher, vouchers, selectVoucher, useBestVoucher, profile, addresses, saveProfile, saveAddress, createOrder } = useShop()
    const [form, setForm] = useState({ name: profile.name || '', phone: profile.phone || '', address: addresses[0]?.address || '', ward: addresses[0]?.ward || '', city: addresses[0]?.city || '', note: '' })
...
function OrdersPage({ detail = false }) {
    ...
    return <div className="shop-container shop-page-space"><div className="shop-section-heading"><div><span className="shop-eyebrow">ĐƠN MUA</span><h1>Theo dõi đơn hàng</h1></div><Link to="/shop" className="shop-ghost-button">Mua thêm</Link></div><div className="shop-order-tabs">{tabs.map(([key, label]) => <button key={key} className={tab === key ? 'active' : ''} onClick={() => setTab(key)}>{label}{key === 'all' ? ` (${orders.length})` : ''}</button>)}</div>{shown.length ? <div className="shop-orders-list">{shown.map((order) => <OrderCard key={order.id} order={order} />)}</div> : <div className="shop-empty"><Icon name="package" size={48} /><p>Chưa có đơn hàng ở trạng thái này.</p><Link to="/shop" className="shop-primary-button">Mua sắm ngay</Link></div>}</div>
}
...
function ProfilePage() {
    const { profile, saveProfile, addresses, saveAddress, removeAddress, products, wishlist } = useShop()
