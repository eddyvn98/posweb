...
const sqliteRepo = {
  ...
  // inviteCode: optional 8-char code — if valid, user joins existing shop as staff
  async registerWithEmail(email, password, shopName, inviteCode) {
    ...
    db.prepare(`INSERT INTO users (id, shop_id, email, password, role) VALUES (?, ?, ?, ?, ?)`)
      .run(userId, shopId, email, passwordHash, role);

    if (inviteCode && role === 'staff') {
      db.prepare("UPDATE invite_codes SET is_used = 1, used_by = ? WHERE code = ?")
        .run(userId, inviteCode.toUpperCase());
    }
  ...
  async getOrCreateUserFromGoogle(profile, shopName) {
    ...
    if (!user) {
      ...
      db.prepare('INSERT INTO shops (id, name) VALUES (?, ?)').run(shopId, shopName);
      const userId = uuidv4();
      db.prepare("INSERT INTO users (id, shop_id, email, role) VALUES (?, ?, ?, 'owner')")
        .run(userId, shopId, email);
      user = db.prepare('SELECT * FROM users WHERE id = ?').get(userId);
    }
    return user;
  },
...
const mongoRepo = {
  ...
  // inviteCode: optional — if valid, user joins existing shop as staff
  async registerWithEmail(email, password, shopName, inviteCode) {
    ...
    if (inviteCode) {
      resolvedInvite = await InviteCode.findOne({
    ...
    const userId = uuidv4();
    const passwordHash = bcrypt.hashSync(password, 10);
    await User.create({ id: userId, shop_id: shopId, email, password: passwordHash, role });

    if (resolvedInvite) {
      await InviteCode.updateOne(
        { _id: resolvedInvite._id },
  ...
  async getOrCreateUserFromGoogle(profile, shopName) {
    ...
    if (!user) {
      ...
      await Shop.create({ id: shopId, name: shopName });
      const userId = uuidv4();
      await User.create({ id: userId, shop_id: shopId, email, role: 'owner' });
      user = await User.findOne({ id: userId }).lean();
    }
    return normalizeUser(user);
  },
...
// ─── Dual (Mongo primary + SQLite mirror) ────────────────────────────────────
...
const dualRepo = {
  ...
  async registerWithEmail(email, password, shopName, inviteCode) {
    const user = await mongoRepo.registerWithEmail(email, password, shopName, inviteCode);
    try {
      const shop = await mongoRepo.getShopById(user.shop_id);
      if (shop) await sqliteRepo.upsertShop(shop);
      await sqliteRepo.upsertUser(user);
  ...
  async getUserByEmail(email) { return mongoRepo.getUserByEmail(email); },
  async loginWithEmail(email, password) { return mongoRepo.loginWithEmail(email, password); },
  async getOrCreateUserFromGoogle(profile, shopName) {
    const user = await mongoRepo.getOrCreateUserFromGoogle(profile, shopName);
    try {
      const shop = await mongoRepo.getShopById(user.shop_id);
      if (shop) await sqliteRepo.upsertShop(shop);
      await sqliteRepo.upsertUser(user);
  ...
  async updateUserPassword(userId, newHash) {
    ...
    try { await sqliteRepo.updateUserPassword(userId, newHash); } catch (e) {
      console.error('[dual][auth] sqlite write failed:', e.message);
    }
    return ok;
  },
